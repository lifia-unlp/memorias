...
export interface ProductionDataPoint {
  ...
  book: number;
  thesis: number; // phdtethis, mastersthesis
  other: number;
  total: number;
}
...
export async function getStatisticsData(): Promise<StatisticsData> {
  ...
  publications.forEach((pub) => {
    const yr = pub.year;
    // We only map in the chart if it is in our range, but we could capture all years.
    // Let's capture all years that have publications, but for the main chart let's focus on the last 10 years.
  ...
  const activeScholarshipsTrend: ScholarshipTrendDataPoint[] = yearsArray.map((yr) => {
    ...
    scholarships.forEach((s) => {
      ...
      const sEnd = s.endDate ? s.endDate.getFullYear() : currentYear; // active up to current if null
  ...
  const members = await prisma.member.findMany({
    select: {
      highestDegree: true,
      positionAtLab: true,
      endDate: true, // to check if active
    },
  });
