...
var chai_exports = {};
(0, import_chunk_2ESYSVXG.__export)(chai_exports, {
  Assertion: () => Assertion,
...
function resetModules(modules, resetMocks = false) {
  ...
  modules.idToModuleMap.forEach((node, path2) => {
...
function createVitest() {
  ...
  const utils = {
    ...
    doUnmock(path2) {
      ...
      _mocker().queueUnmock(path2, importer);
    },
    async importActual(path2) {
      const importer = getImporter("importActual");
      return _mocker().importActual(path2, importer, _mocker().getMockContext().callstack);
    },
    async importMock(path2) {
      const importer = getImporter("importMock");
      return _mocker().importMock(path2, importer);
    },
