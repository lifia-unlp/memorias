...
* **Tasks**:
  - [ ] Analyze the Prisma configuration (`prisma/schema.prisma`) and database migration history.
  - [ ] Initialize the [`Shared-Domain-Glossary.md`](../../memorias-wiki/Shared-Domain-Glossary.md) with absolute definitions for core entities: `User`, `Member`, `Project`, `Thesis`, `Scholarship`, `Publication`, `SystemOption`, `AuditLog`, `SystemSetting`, `Report`.
...
* **Tasks**:
  - [ ] Analyze sign-in, sign-up, and provider integration flows.
  - [ ] Document the account activation workflow (`pending-activation`) and role-based permissions (`USER`, `EDITOR`, `ADMIN`).
  - [ ] Document user preferences (notification subscriptions, digest email toggle).
  - [ ] Scenarios: *Sign In*, *Activation Request*, *Update Preferences*.
...
* **Tasks**:
  - [ ] Extract the UI layouts and forms for creating, viewing, and modifying member profiles.
  - [ ] Detail professional fields, custom categories, UNLP/CONICET/CIC dropdown roles.
  - [ ] Document CV fields, short bios, and contact links (ORCID, Google Research, ResearchGate).
  - [ ] Scenarios: *Create Profile*, *Edit Professional Roles*, *Link User to Member*.
...
* **Tasks**:
  - [ ] Map the progress tracking logic (10% to 100% in intervals of 10).
  - [ ] Analyze thesis level fields (PhD, Masters, Grade) and student/director mappings.
  - [ ] Document thesis report uploads and external report links.
  - [ ] Scenarios: *Register Thesis*, *Update Thesis Progress*, *Filter Theses by Level*.

#### Module E: Scholarships Management
* **Target Files**: `src/app/scholarships/`, `src/components/scholarships/`.
* **Tasks**:
  - [ ] Map scholarship definitions, funding agencies, and duration limits.
  - [ ] Analyze relationships linking scholarships to projects and theses.
  - [ ] Scenarios: *Add Scholarship*, *Associate Scholarship to Project*.

#### Module F: Publications & BibTeX Parsing Engine
* **Target Files**: `src/app/publications/`, `src/lib/bibtex/` (or equivalent parsing utils).
* **Tasks**:
  - [ ] Analyze how BibTeX files are imported and parsed into JSON storage (`bibtexData`).
  - [ ] Document custom fields like Rank/Indexing and Self-Archiving URLs.
  - [ ] Document publications dashboard filters (year, type, author, tag).
  - [ ] Scenarios: *Import BibTeX Publication*, *Search Publications by Author*, *Toggle Featured Publication*.
